...
class Planner(torch.nn.Module):
    def __init__(self, channels=[16, 32, 32, 32]):
        ...
        input_channels, conv_layers = 3, []
        for output_channels in channels:
            conv_layers += conv_block(output_channels, input_channels)
            input_channels = output_channels

        self._conv = torch.nn.Sequential(*conv_layers, torch.nn.Conv2d(input_channels, 1, 1))
