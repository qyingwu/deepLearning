...
def extract_peak(heatmap, max_pool_ks=7, min_score=-5, max_det=100):
    ...
    mask = (max_cls > heatmap).float()
    possible_det = heatmap - mask * 1e5
    max_det = min(max_det, possible_det.numel())
    score, location = torch.topk(possible_det.view(-1), max_det)
    peak_list = []
    for s, l in zip(score.cpu(), location.cpu()):
        if s > min_score:
          s_float = float(s)
          l_mod = int(l) % heatmap.size(1)
          l_div = int(l) // heatmap.size(1)
          peak_list.append((s_float, l_mod, l_div))
    return peak_list
...
class Detector(torch.nn.Module):
    ...
    def detect(self, image, **kwargs):
        ...
        cls_scores, obj_sizes = self.forward(image[None])
        obj_sizes = obj_sizes.cpu()
        detections = []
        for class_scores in cls_scores[0]:
          class_detections = extract_peak(class_scores, max_det=30, **kwargs)
          detections.append([(score, cx, cy, float(obj_sizes[0, 0, cy, cx]), float(obj_sizes[0, 1, cy, cx])) for score, cx, cy in class_detections])
