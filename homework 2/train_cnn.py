...
def train(args):
    ...
    transform = eval(args.transform, {k: v for k, v in inspect.getmembers(torchvision.transforms) if inspect.isclass(v)})
    ...
    for epoch in range(args.num_epoch):
        ...
        if valid_logger is None or train_logger is None:
            print('epoch %-3d \t acc = %0.3f \t val acc = %0.3f' % (epoch, confusion.global_accuracy, val_confusion.global_accuracy))
        save_model(model)
